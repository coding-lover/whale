...
class Server
{
    ...
    {
        // 启用 Swoole PDO Hook，让 Eloquent ORM 使用的 PDO 查询协程化、不阻塞事件循环
        // 仅 Hook PDO + TCP，避免影响 cURL（交易所适配器使用原生协程 HTTP 客户端）
        if (function_exists('Swoole\\Runtime::enableCoroutine')) {
            \Swoole\Runtime::enableCoroutine(SWOOLE_HOOK_PDO | SWOOLE_HOOK_TCP);
        }
