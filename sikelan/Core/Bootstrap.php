...
class Bootstrap
{
     ...
     *
     * 依次执行：core 函数预加载 → autoload → core → xdebug 关闭。
     * ⚠️ constants.php + common.php 必须在 autoload 之前加载，
     *    确保框架的 env() 先于 illuminate/support 的 env() 定义，
     *    避免 illuminate 的 env() 覆盖框架实现。
     *
     * @param string $entryDir 入口文件所在目录，一般传 `__DIR__`。
     * @param bool   $warnXdebugCLI  true = 打印 xdebug 警告；false = 静默关
     */
    public static function cli(string $entryDir, bool $warnXdebugCLI = true): void
    {
        self::preloadCoreFunctions($entryDir);
    ...
    /**
     * 预加载核心函数（constants.php + common.php）。
     *
     * 在 composer autoload 之前调用，确保框架的 env() 等全局函数
     * 优先于第三方库（如 illuminate/support）的同名函数定义。
     *
     * @param string $baseDirOrEntryDir 项目根路径或入口目录
     */
    public static function preloadCoreFunctions(string $baseDirOrEntryDir): void
    {
