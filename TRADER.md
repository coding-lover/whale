...
- [四、策略参数调优](#四策略参数调优)
  - [4.1 核心原则：别只盯总收益率](#41-核心原则别只盯总收益率)
  - [4.2 调优流程（三阶段网格 + 样本内外检验）](#42-调优流程三阶段网格--样本内外检验)
  - [4.3 实战案例：WmcStrategy（EMA 金叉死叉）调优](#43-实战案例wmcstrategyema-金叉死叉调优)
  - [4.4 调优总结与方法论](#44-调优总结与方法论)
  - [4.5 自己动手：参数寻优脚本要点](#45-自己动手参数寻优脚本要点)
- [附录](#附录)
  - [A. 目录与模块总览](#a-目录与模块总览)
...
> 策略写出来只是"能跑"，参数调对了才"能赚"。本章以 **WmcStrategy（EMA 金叉死叉）** 在
> `SKR/USDT:SWAP` 15m、30 天数据上的真实调优过程为例，讲清楚**怎么调、怎么避免自欺欺人**。

## 4.1 核心原则：别只盯总收益率

调参最大的陷阱是：**把历史回测收益调到最高，然后实盘亏钱**。因为参数可以被"过拟合"到
刚好契合过去这段行情的偶然波动。

**单一指标（尤其是总收益率）一定会骗你。** 调参时要同时看：

| 维度 | 指标 | 为什么重要 |
|------|------|-----------|
| 盈利能力 | 总收益率 / 年化 | 目标，但**不能单独看** |
| 风险调整后收益 | **夏普比率**、索提诺、卡玛 | 单位风险赚的钱，比裸收益更可靠 |
| 交易质量 | **利润因子(PF)**、**盈亏比(PLR)** | PF=毛利/毛损，>1.5 才算健康；盈亏比=平均盈利/平均亏损 |
| 风控 | **最大回撤** | 收益再高，回撤 30% 也拿不住 |
| 统计显著性 | 交易笔数 | <10 笔的高收益基本是运气，没有统计意义 |
| **稳健性** | **样本内/样本外分段都盈利** | ⭐ 防过拟合的核心手段（见 4.2） |

> **一句话原则：宁可要"样本内外都赚、回撤小"的中等收益，也不要"只在一段行情爆赚、回撤巨大"的最高收益。**

## 4.2 调优流程（三阶段网格 + 样本内外检验）

### 第 0 步：跑基线，先诊断再动手

先用当前参数跑一次，**找出"哪里不对"**，而不是无脑扫参数。

```bash
php bin/sikelan trader:backtest --strategy=WmcStrategy \
    --symbol=SKR/USDT:SWAP --timeframe=15m --days=30 --warmup=5 --json
```

WmcStrategy 基线结果：

```
总收益率 +1.75% | 胜率 70% | 盈亏比 0.96 | 利润因子 2.25 | 最大回撤 0.70% | 20 笔
```

**诊断关键**：胜率高达 70%，但**盈亏比只有 0.96（< 1）**——意味着平均每笔盈利 < 平均每笔亏损。
这是典型的 **"截断利润、让亏损奔跑"**：赢的单子被早早止盈，输的单子放任到止损。
高胜率掩盖了"赚小亏大"的结构问题。→ 优化方向应放在**出场/止盈止损**，而不只是入场参数。

### 第 1 步：入场参数粗扫（EMA 周期）

固定风控参数，对**构造参数**做网格（本例 `emaShort × emaLong × filterPct`）：

```
short  ∈ [5, 8, 10, 12, 15]
long   ∈ [20, 25, 30, 40, 50, 60, 80, 100]   (须 > short)
filter ∈ [0, 0.001, 0.003, 0.005, 0.008]
```

按总收益排序取 Top N（本例 Top 12）。这一步只是**缩小范围**，不做最终决定。

### 第 2 步：风控参数细扫

对第 1 步的 Top 组合，再叠加**风控网格**：

```
stoploss  ∈ [2%, 3%, 5%, 8%]
trailing  ∈ [关, 3%]
ROI 阶梯  ∈ tight(1%起) / wide(2%起) / off(关闭，只靠信号+止损)
```

### 第 3 步：⭐ 样本内外稳健性检验（防过拟合）

把数据按时间切成两段：

- **样本内（in-sample）**：前 2/3 数据
- **样本外（out-of-sample）**：后 1/3 数据

对每个候选参数组合，分别在**全段 / 样本内 / 样本外**跑回测，**只保留两段都盈利的组合**。

> 为什么有效？过拟合参数的特征就是"在一段行情完美、换一段就崩"。样本外数据相当于
> 策略没"见过"的考题，两段都过才算真本事。

### 第 4 步：邻近微调确认局部最优

对胜出组合，在其邻近参数（EMA±、止损±、ROI 阶梯变体）再细扫一轮，确认它是"平台"而非"孤峰"。
**孤峰参数（周围参数都差）往往是巧合，平台参数（邻近参数都不错）才稳健。**

## 4.3 实战案例：WmcStrategy（EMA 金叉死叉）调优

### 调优中的关键一幕：最高收益组合被否决

第 1~2 轮网格里，收益最高的组合是 **"关闭 ROI 止盈 + 2% 止损"**：

```
全段 +13.90%（远超其他组合）  但：样本内 -1.19% / 样本外 +12.41% | 最大回撤 9.35% | 胜率仅 31%
```

**它的利润 100% 来自最后 10 天的一波趋势，前 20 天是亏的。** 这正是过拟合的教科书特征——
靠"拿到一波大趋势"堆收益，但平时持续小亏、回撤巨大。**果断否决。**

### 最终入选参数

在 1728 个组合中，唯一满足"**样本内外两段都盈利 + 回撤 < 1%**"的组合：

| 参数 | 调优前 | **调优后** | 依据 |
|------|--------|-----------|------|
| `emaShort` | 20 | **12** | 15m 短周期下更灵敏的金叉 |
| `emaLong` | 50 | **60** | 更长的趋势过滤，减少假信号 |
| `filterPct` | 0.003 | **0.005** | 更严格的收盘确认，过滤假金叉 |
| `stoploss` | 3% | **1.5%** | 紧止损快速认错，趋势反转交给 EMA 死叉 |
| `minimalRoi` | 1%/0.8%/0.5%/0（紧） | **3%/2%/1%/0.5%**（宽） | ⭐ 核心修复：让利润奔跑 |
| `trailingStop` | 0 | **0** | 网格实测追踪止损**全部更差**，不启用 |
...
| 总收益率 | +1.75% | **+2.27%** | +30% |
| 年化收益 | +23.6% | **+31.5%** | |
| **盈亏比** | **0.96** | **1.76** | ⭐ 从"赚小亏大"到"赚大亏小" |
| 利润因子 | 2.25 | 2.29 | |
| 夏普比率 | 5.26 | **6.37** | 风险调整后收益更高 |
| 最大回撤 | 0.70% | **0.58%** | 回撤反而更小 |
| 样本内（前20天） | **−0.40%（亏）** | **+0.68%** | ⭐ 两段都盈利 |
| 样本外（后10天） | +2.12% | +1.42% | |

> 注意：调优后胜率从 70% **降到** 56.5%，但盈亏比从 0.96 升到 1.76——**收益反而更高**。
> 这印证了量化交易的常识：**胜率不重要，盈亏比 × 胜率（期望值）才重要。** 少赢几次、
> 但每次赢大的，远胜于频繁小赢、偶尔大亏。
...
1. **先诊断，后调参。** 看基线的盈亏比/PF/回撤找结构问题（本例是"止盈太早"），
   对症下药比暴力扫参高效得多。

2. **盈亏比比胜率重要。** 高胜率 + 盈亏比<1 是陷阱（赚小亏大）；
   中等胜率 + 盈亏比>1.5 才是健康系统。期望值 = 胜率 × 平均盈利 − 败率 × 平均亏损。

3. **出场参数和入场参数同等重要，甚至更重要。** 本例收益改善主要来自**放宽 ROI 止盈 + 收紧止损**
   （让利润奔跑、快速截断亏损），而不是 EMA 周期本身。

4. **追踪止损/止盈不是越紧越好。** 紧止盈在短周期（15m）噪音里会被频繁扫出，
   砍掉盈利单。每个参数都要用数据验证，不要凭直觉。

5. **必须做样本外检验。** 收益最高的组合往往是过拟合最严重的。用"两段都盈利 + 低回撤"
   做筛选标准，牺牲一点账面收益换稳健性。

### 调参的纪律（红线）

- ✅ **交易笔数要够**（建议 ≥ 15~20 笔），否则统计无意义
- ✅ **参数落在"平台"而非"孤峰"**（邻近参数表现也要不差）
- ✅ **一个策略/一个币种调出来的参数，不要盲目套用到别的币种/周期**
- ❌ 不要为了刷收益不断加参数维度（维度越多越容易过拟合）
- ❌ 不要用同一批数据既调参又验证（必须切分样本内外）

### 实盘前的额外建议

历史回测（即使有样本外验证）仍是"对过去的拟合"。上实盘前：

1. 拉**更长时间**数据（90/180 天）跨多种行情（牛/熊/震荡）再验一次
2. 先用 **dry-run 模式**跑 1~2 周，对比实盘信号与回测是否一致
3. 小资金起步，实盘滑点/手续费/深度都比回测假设更差

## 4.5 自己动手：参数寻优脚本要点

项目没有内置 hyperopt 命令，调参用一次性 PHP 脚本即可（数据加载一次，内存里批量回测）。
关键骨架：

```php
// 1) 数据只加载一次（避免重复读 CSV）
$dp = BacktestServiceProvider::loadDataProvider('binance', 'SKR/USDT:SWAP', '15m', [], false);

// 2) 需要调"风控属性"（stoploss/minimalRoi 是 protected）时，用匿名子类覆写
$strat = new class($short, $long, $filter, $stop, $roi) extends WmcStrategy {
    public function __construct($s, $l, $f, $stop, $roi) {
        parent::__construct($s, $l, $f);
        $this->stoploss   = $stop;     // 覆写 protected 风控属性
        $this->minimalRoi = $roi;
    }
};

// 3) 跑一次回测，取关键指标
$bt  = BacktestServiceProvider::newBacktesting(container(), $dp, $strat, ['warmup_candles' => 5]);
$m   = (new PerformanceReport($bt->run(null, null, $from, $to), 10000.0, 365))->all();
// $m['total_return_pct'] / $m['profit_factor'] / $m['max_drawdown_pct'] / $m['sharpe_ratio'] ...

// 4) 网格 foreach 组合参数 → 收集结果 → usort 排序
// 5) 用 $dp->getCandles() 拿到首尾 K 线时间戳，切 2/3 分样本内外分别 run(from,to)
```

> 提示：回测入口 `run(?array $symbols, ?string $timeframe, ?int $fromMs, ?int $toMs)`
> 的第 3、4 参数就是时间窗口（毫秒），样本内外分段就是传不同的 `fromMs/toMs`。
