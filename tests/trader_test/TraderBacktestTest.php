...
class TraderBacktestTest extends TestCase
{
    ...
    // ---- 11. getMatrices / getMatrix：暴露策略 populate 后的矩阵 ----

    public function testGetMatricesThrowsBeforeRun(): void
    {
        $dp = new ArrayDataProvider();
        $symbol = TradingSymbol::parse('BTC/USDT');
        $dp->setCandles($symbol, '5m', $this->makeTrendCandles(180, 300_000));
        ...
        // 未 run() 直接调用 → 抛异常（明确告知调用顺序）
        $this->expectException(\RuntimeException::class);
        $this->expectExceptionMessage('Call run() first');
        $backtest->getMatrices();
    }

    public function testGetMatrixReturnsNullBeforeRun(): void
    {
        $dp = new ArrayDataProvider();
        $symbol = TradingSymbol::parse('BTC/USDT');
        $dp->setCandles($symbol, '5m', $this->makeTrendCandles(180, 300_000));
        ...
        // 未 run() 调用单 symbol 查询 → null（不抛异常，方便链式调用）
        $this->assertNull($backtest->getMatrix('BTC/USDT'));
        $this->assertNull($backtest->getMatrix($symbol));
    }

    public function testGetMatricesReturnsAllSymbolsAfterRun(): void
    {
        if (!extension_loaded('trader')) {
            $this->markTestSkipped('缺少 trader 扩展，跳过 getMatrices 测试。');
        }
        $dp = new ArrayDataProvider();
        $btc = TradingSymbol::parse('BTC/USDT');
        $eth = TradingSymbol::parse('ETH/USDT');
        $dp->setCandles($btc, '5m', $this->makeTrendCandles(180, 300_000));
        $dp->setCandles($eth, '5m', $this->makeTrendCandles(180, 300_000));

        $strategy = new EmaCrossStrategy(5, 15, 0.0);
        $backtest = BacktestServiceProvider::build($this->minimalBacktestConfig(), $dp, $strategy, null);
        $backtest->run();

        // 返回 key 必须是 (string)symbol，包含两个 symbol
        $matrices = $backtest->getMatrices();
        $this->assertCount(2, $matrices);
        $this->assertArrayHasKey('BTC/USDT', $matrices);
        $this->assertArrayHasKey('ETH/USDT', $matrices);

        // 矩阵行数 == K 线数（180 根，未被裁剪）
        $this->assertSame(180, count($matrices['BTC/USDT']));
        $this->assertSame(180, count($matrices['ETH/USDT']));

        // 每行至少 12 列（OHLCV + 信号，SignalCols::NUM_COLUMNS）
        $firstRow = $matrices['BTC/USDT'][0];
        $this->assertGreaterThanOrEqual(SignalCols::NUM_COLUMNS, count($firstRow));

        // SignalCols::CLOSE (4) 列必须等于第一根 K 线的 close（100.0，makeTrendCandles 的起点）
        $this->assertSame(100.0, (float) $firstRow[SignalCols::CLOSE]);

        // SignalCols::DATE (0) 必须是合法毫秒时间戳
        $this->assertGreaterThan(0, (int) $firstRow[SignalCols::DATE]);
    }

    public function testGetMatrixReturnsSingleSymbolAndSupportsBothArgTypes(): void
    {
        if (!extension_loaded('trader')) {
            $this->markTestSkipped('缺少 trader 扩展，跳过 getMatrix 测试。');
        }
        $dp = new ArrayDataProvider();
        $symbol = TradingSymbol::parse('BTC/USDT');
        $dp->setCandles($symbol, '5m', $this->makeTrendCandles(180, 300_000));
        ...
        // 入参为字符串
        $byString = $backtest->getMatrix('BTC/USDT');
        $this->assertNotNull($byString);
        $this->assertSame(180, count($byString));

        // 入参为 TradingSymbol 对象
        $byObj = $backtest->getMatrix($symbol);
        $this->assertNotNull($byObj);
        $this->assertSame($byString, $byObj, '字符串入参和对象入参返回的矩阵应一致');

        // 不存在的 symbol → null（不抛异常）
        $this->assertNull($backtest->getMatrix('DOGE/USDT'));
    }

    public function testRunOverwritesPreviousMatrices(): void
    {
        if (!extension_loaded('trader')) {
            $this->markTestSkipped('缺少 trader 扩展，跳过矩阵覆盖测试。');
        }
        $dp = new ArrayDataProvider();
        $btc = TradingSymbol::parse('BTC/USDT');
        $eth = TradingSymbol::parse('ETH/USDT');
        ...
        // 第一次 run()：跑两个 symbol
        $backtest->run();
        $this->assertCount(2, $backtest->getMatrices());

        // 第二次 run()：只跑一个 symbol → 之前的 BTC/USDT 矩阵应被覆盖（只剩 ETH/USDT）
        $backtest->run([$eth], '5m');
        $matrices = $backtest->getMatrices();
        $this->assertCount(1, $matrices, 'run() 应覆盖上次矩阵，只保留本次的 symbol');
        $this->assertArrayNotHasKey('BTC/USDT', $matrices);
        $this->assertArrayHasKey('ETH/USDT', $matrices);
    }
