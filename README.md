...
控制器支持**方法注入**：在方法参数上用类型提示声明服务，框架会自动从容器解析并注入，无需手动 `new`。
...
{
    // 方法注入：OrderService / PaymentService 由容器自动装配
    public function store(Request $request, OrderService $orderService, PaymentService $paymentService)
    {
        $data = $request->getPostParams();
        $order = $orderService->create($data);
        $paymentService->pay($order['id']);

        return ['message' => '下单成功', 'order' => $order];
    }

    // 路由参数也可按参数名注入（如 /orders/{id}）
...
> 参数解析优先级：`Request` 类型 → 类/接口类型（容器解析）→ 路由参数按名匹配 → 旧签名 `$params` → 默认值 → `null`。
...
框架支持四种获取服务实例的方式，按需选用：

**① 构造函数注入（最推荐）** —— 依赖关系一目了然，由容器自动装配。
...
**② 属性注入** —— 在成员变量上声明类型，容器自动解析并注入，无需写构造函数。

```php
class TestServicesProvider
{
    // 容器自动从容器解析 ExchangeManager 并注入此属性
    private ExchangeManager $exchangeManager;

    public function test()
    {
        return $this->exchangeManager->exchange('binance')->formatSymbol('BTC/USDT:quarter');
    }
}
```

> 属性注入规则：仅对**未初始化**的非内置类型属性生效。已在构造函数中赋值或声明了默认值的属性不会被覆盖。

**③ 方法注入** —— 只在某个控制器方法里用到时，在参数上类型提示即可，框架自动解析。

```php
public function store(Request $request, OrderService $orderService)
{
    $orderService->create($request->getPostParams());
}
```

**④ 手动从容器取** —— 在非容器管理的普通类/闭包里，用 `app(Class::class)` 或 `container()->get()`。
...
// 不传参时 app() 返回 Framework 单例（兼容旧用法）
$framework = app();
```
...
框架集成了 [illuminate/database](https://github.com/illuminate/database)（Eloquent ORM），
提供与 Laravel 一致的 ActiveRecord 体验，且针对 Swoole 协程做了连接池适配。

#### 架构

| 组件 | 职责 |
|------|------|
| `Sikelan\Database\PdoPool` | 协程感知的 PDO 连接池，每协程独占连接，协程结束自动归还 |
| `Sikelan\Database\CoroutineMySqlConnection` | 继承 Eloquent 的 MySqlConnection，从 PdoPool 取连接 |
| `Sikelan\Database\EloquentManager` | 封装 Capsule 初始化，框架启动时自动 boot |
| `App\Models\Model` | 应用层 Model 基类，继承 EloquentModel，默认关闭时间戳 |

框架启动时自动完成 Eloquent 初始化（`Framework::initEloquent()`），
并在服务入口启用 `Swoole\Runtime::enableCoroutine(SWOOLE_HOOK_PDO)` 让 PDO 查询非阻塞。
...
- 每个协程从 PdoPool 获取独立的 PDO 连接，存放在协程上下文中
- 同一协程内多次查询复用同一连接，协程结束自动归还
- PdoPool 懒加载，无 DB 环境下框架启动不会失败
- 禁止在协程间共享 PDO 连接或 Model 查询结果中的连接引用

## 安全处理

框架内置参考 Laravel / Symfony 设计的输入输出安全处理组件（位于 `sikelan/Security/`），遵循"默认安全优先"原则——不需要额外配置即可获得基础防护。

| 组件 | 职责 | 快捷方式 |
|------|------|----------|
| `InputSanitizer` | 输入净化（去 null byte / 控制字符）+ 类型转换 | `$request->input()` 等便利方法 |
| `HtmlEncoder` | HTML 转义（防 XSS） | `e($value)` |
| `Validator` | 流式数据验证 | `validator($data, $rules)` |
| `Hasher` | 密码哈希（bcrypt） | `bcrypt()` / `bcrypt_verify()` |
| `Crypto` | 对称加密（AES-256-GCM） | `encrypt()` / `decrypt()` |
| `SecurityHeaders` | 默认安全响应头 | 响应自动注入，无需手动调用 |

### 输入净化与安全获取

`Request` 提供一系列带净化和类型转换的取值方法，**优先级与 `getParam()` 一致：query > post > route**：

```php
public function store(Request $request, $params)
{
    // 通用取值：净化 + 可选 cast（int|float|bool|string|email|url|alpha|alnum）
    $keyword = $request->input('keyword');                        // 净化后的原始值
    $status  = $request->input('status', 'active', 'alnum');      // cast 校验失败返回 default

    // 类型安全取值（校验失败返回默认值，不抛异常）
    $page    = $request->getInt('page', 1);        // int，非数字返回 1
    $amount  = $request->getFloat('amount', 0.0);  // float
    $name    = $request->getString('name', '');    // string，数组返回 ''
    $enabled = $request->getBool('enabled', false);// 兼容 "1"/"true"/"yes"/"on"
    $email   = $request->getEmail('email');        // ?string，非法邮箱返回 null
    $url     = $request->getUrl('url');            // ?string，非法 URL（含 javascript:）返回 null
    $code    = $request->getAlpha('code', '');     // 仅保留 a-zA-Z
    $sku     = $request->getAlnum('sku', '');      // 仅保留 a-zA-Z0-9

    // 批量操作
    $has    = $request->has('keyword');            // 是否存在（三源任一）
    $subset = $request->only(['name', 'email']);   // 白名单取值
    $safe   = $request->except(['password']);      // 排除敏感字段
    $all    = $request->all();                     // 三源合并 + 全量净化
}
```

**净化规则**：所有便利方法自动去除 null byte（防截断攻击）和控制字符，字符串保留 `\t`、`\n`、`\r`；数组递归处理，非字符串（int/float/bool/null）原样返回。

### 输出转义与安全响应

**HTML 转义**（防 XSS）——输出到 HTML 前必须转义：

```php
// 全局函数 e()：ENT_QUOTES | ENT_SUBSTITUTE，UTF-8
$name = e($request->input('name', 'Guest'));

// 递归转义数组/对象中的所有字符串（用于视图数据批量转义）
...
return (new Response())->withJson(['html' => $userInput], true);
```

**安全响应头**——`RequestHandler` 自动为所有响应注入，无需手动调用；也可手动覆盖：
...
默认注入的头（可在 `config/security.php` 的 `headers` 中覆盖或禁用）：
...
流式验证器，规则用 `|` 分隔，参数用 `:` 传递，风格与 Laravel 一致：

```php
$validator = validator($request->getPostParams(), [
    'name'     => 'required|string|min:2|max:50',
    'email'    => 'required|email',
    'age'      => 'nullable|int|between:1,120',       // nullable：null 时跳过后续规则
    'password' => 'required|min:8|confirmed',         // 要求 password_confirmation 字段一致
    'role'     => 'required|in:admin,user',           // 枚举
    'phone'    => 'regex:1[3-9]\d{9}',                // 正则（分隔符内 / 需转义）
]);
...
| 规则 | 说明 |
|------|------|
| `required` | 必填（null / 空字符串 / 空数组都算未通过） |
| `nullable` | 允许为 null，null 时跳过该字段其余规则 |
| `int` / `string` / `email` / `url` | 类型校验 |
| `regex:pattern` | 正则匹配 |
| `min:n` / `max:n` | 数值大小 / 字符串长度（按 mb_strlen 计算） |
| `between:min,max` | 数值或长度区间 |
| `in:a,b,c` / `not_in:a,b,c` | 枚举白名单 / 黑名单 |
| `alpha` / `alnum` | 纯字母 / 字母数字 |
| `confirmed` | 与同字段加 `_confirmation` 后缀的值一致（密码二次确认） |

自定义错误消息（按 `字段名.规则名` 键覆盖默认的中文消息，消息原样使用）：

```php
$validator = validator($data, ['email' => 'required|email'], [
...
基于 bcrypt（自动加盐），禁止在业务中自行使用 `md5` / `sha1` 存密码：

```php
// 注册：哈希密码（cost 取 config/security.php 的 bcrypt_cost，默认 10）
$hash = bcrypt($plainPassword);

// 登录：验证
if (bcrypt_verify($inputPassword, $user['password_hash'])) {
    // 登录成功
}

// 登录成功后可检查是否需要升级 cost（平滑迁移）
if (\Sikelan\Security\Hasher::needsRehash($user['password_hash'], ['cost' => 12])) {
    $this->db->update('users', ['password_hash' => bcrypt($inputPassword)], ['id' => $user['id']]);
}
```

### 对称加密

AES-256-GCM 加解密（带认证标签，防篡改），用于加密 API 密钥、Token 等敏感数据：

```php
// 加密（自动读取 .env 中的 APP_KEY，随机 nonce，输出 base64）
$cipher = encrypt('my-secret-api-key');

// 解密（密钥不匹配 / 数据被篡改 / 格式错误一律返回 null，不抛异常）
$plain = decrypt($cipher);  // 'my-secret-api-key' 或 null
```

首次使用前生成 APP_KEY：

```bash
# 生成 32 字节随机密钥（base64），自动写入 .env 的 APP_KEY（已存在则替换）
php bin/sikelan security:key
```

> ⚠️ **密钥轮换警告**：更换 APP_KEY 后，旧密钥加密的数据将无法解密。生产环境轮换密钥前，必须先用旧密钥解密并重新加密存量数据。

### SQL 标识符转义

参数化查询只能保护**值**，表名 / 列名无法参数化。框架的 `insert()` / `update()` / `delete()` 已自动对表名和列名做标识符转义（白名单校验 + 反引号包裹），无需手动处理；手写 SQL 拼接标识符时请使用：

```php
$db = db();  // 公用函数，等价于 app()->getDb()

// ✅ 动态表名/列名：先校验再包裹（非法字符直接抛 InvalidArgumentException）
$table = $db->quoteIdentifier($tableName);   // 输出: `orders`
$col   = $db->quoteIdentifier($columnName);  // 输出: `user_id`

$sql = "SELECT {$col} FROM {$table} WHERE id = ?";  // 值仍走参数化
```

> 合法标识符：字母或下划线开头，仅含字母、数字、下划线（`/^[a-zA-Z_][a-zA-Z0-9_]*$/`）。表名来自用户输入时，转义校验会直接抛异常，这是预期的防护行为。

### 安全配置

所有安全行为可通过 `config/security.php` 调整，支持 `.env` 环境变量覆盖：

```php
return [
    // 安全响应头（单项设为 false/null 关闭，设为字符串覆盖默认值）
    'headers' => [
        'X-Content-Type-Options' => 'nosniff',
        'X-Frame-Options'        => 'SAMEORIGIN',
        'Referrer-Policy'        => 'no-referrer-when-downgrade',
    ...
    // bcrypt cost（4-31，建议生产环境 10-12，每 +1 耗时约翻倍）
...
`make:model` 以**数据库表**为输入：先查 `information_schema` 校验表是否存在（不存在直接退出），
再读取表结构自动生成 Eloquent Model（类名、`$table`、`$primaryKey`、`$fillable`、`$casts`、`$timestamps` 全部自动填充）。

```bash
# 按表名生成（users → User，order_items → OrderItem，categories → Category）
php bin/sikelan make:model users

# 指定 Model 类名（适用于表名不规则的场景）
...
| Model 属性 | 推导来源 |
|------------|----------|
| 类名 | 表名单数化 + PascalCase（`--model` 可覆盖） |
| `$table` | 输入的表名 |
| `$primaryKey` | 主键列（非 `id` 才输出；UUID 字符串主键自动设 `keyType='string'` + `incrementing=false`） |
| `$fillable` | 除主键外的全部列 |
| `$casts` | `tinyint(1)`→boolean、整型→integer、`decimal(p,n)`→`decimal:n`、json→array、datetime→datetime 等 |
| `$timestamps` | 同时存在 `created_at` + `updated_at` 列时自动开启 |

> 命令依赖数据库连接，请确认 `.env` 中 `DB_HOST / DB_USERNAME / DB_PASSWORD / DB_DATABASE` 配置正确。
