...
class SignalCols
{
    ...
    /**
     * 返回 rowToAssoc 的 key 列表（即 12 列标准列名，顺序与 rowToAssoc 一致）
     *
     * 典型用途：
     *   - 导出 CSV / 数据库表的表头
     *   - API 响应声明列名
     *   - 对矩阵做列名化时的 schema 声明
     *
     * 注意：此列表与 rowToAssoc() 的 key 保持一致；修改 rowToAssoc 时须同步更新本方法。
     *
     * @return array<int, string> 列名列表，顺序对应 matrix 的列下标 0..11
     */
    public static function assocKeys(): array
    {
        return [
            'date',
            'open',
