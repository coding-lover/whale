...
class Backtesting implements LoggerAwareInterface
{
     ...
     * 最近一次 run() 生成的策略矩阵
     *
     * key = (string)symbol；value = list<list<mixed>>，每行一根 K 线，
     * 至少 12 列（OHLCV + 信号，列下标见 SignalCols 常量），
     * 策略 populateIndicators 可能附加更多指标列（RSI / EMA / BB 等）。
     *
     * @var array<string, array<int, array<int, mixed>>>|null
     */
    private $lastMatrices = null;
    ...
    public function run(?array $symbols = null, ?string $timeframe = null, ?int $fromMs = null, ?int $toMs = null): BacktestResult
    {
        ...
        // 保存矩阵：供 getMatrices() / getMatrix() 查询（信号可视化、调试策略逻辑、绘图等场景）
        $this->lastMatrices = [];
        foreach ($prepared as $symStr => $p) {
            $this->lastMatrices[$symStr] = $p['matrix'];
        }
    ...
    // -----------------------------------------------------------
    //  矩阵访问器：暴露 populateIndicators/EntryTrend/ExitTrend 后的矩阵
    // -----------------------------------------------------------

    /**
     * 获取最近一次 run() 生成的全部策略矩阵
     *
     * 矩阵结构：`list<list<mixed>>`，每行一根 K 线，每行至少 12 列（OHLCV + 信号），
     * 策略 populateIndicators 可能附加更多指标列（RSI / EMA / BB 等）。
     * 列下标含义见 {@link SignalCols} 常量。
     *
     * 典型用途：
     *   - 信号可视化（绘制 K 线 + 标记 enter_long / exit_long 触发位置）
     *   - 调试策略逻辑（检查指标数值是否符合预期）
     *   - 提取指标序列用于绘图或导出
     *
     * 注意：每次 run() 会覆盖前一次的矩阵。多协程并发使用同一 Backtesting 实例
     * 不安全（run() 本身会重置全部运行时状态）。
     *
     * @return array<string, array<int, array<int, mixed>>> key = (string)symbol，value = matrix
     * @throws \RuntimeException 尚未运行过 run()
     */
    public function getMatrices(): array
    {
        if ($this->lastMatrices === null) {
            throw new \RuntimeException(
                '[trader] No matrices available. Call run() first.'
            );
        }
        return $this->lastMatrices;
    }

    /**
     * 获取指定 symbol 的策略矩阵
     *
     * @param TradingSymbol|string $symbol 交易对（TradingSymbol 会自动转 string）
     * @return array<int, array<int, mixed>>|null matrix；未 run() 或 symbol 不存在返回 null
     */
    public function getMatrix($symbol): ?array
    {
        if ($this->lastMatrices === null) {
            return null;
        }
        $key = is_string($symbol) ? $symbol : (string) $symbol;
        return $this->lastMatrices[$key] ?? null;
    }
