...
class ExchangeController
{
     ...
     *
     * @param Request $request HTTP 请求对象（路由参数 exchange / symbol 通过 $request->input() 获取）
     * @return Response JSON 响应
     */
    public function getTicker(Request $request): Response
    {
     ...
     * @return Response JSON 响应
     */
    public function getOrderBook(Request $request): Response
    {
        $exchangeName = $request->input('exchange', 'binance');
        $symbol = $request->input('symbol', '');
     ...
     * @return Response JSON 响应
     */
    public function getKlines(Request $request): Response
    {
        $exchangeName = $request->input('exchange', 'binance');
        $symbol = $request->input('symbol', '');
     ...
     * @return Response JSON 响应
     */
    public function getTrades(Request $request): Response
    {
        $exchangeName = $request->input('exchange', 'binance');
        $symbol = $request->input('symbol', '');
        $limit = (int) ($request->getQueryParams()['limit'] ?? 100);
